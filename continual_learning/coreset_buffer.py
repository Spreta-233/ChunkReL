...
class CoresetBuffer(object):
    def __init__(self, local_path, model_params, transforms, selection_params, buffer_size, use_cuda,
        ...
        self.coreset_selector = selection_agent.RhoSelectionAgent(
            ...
            selection_strategy=self.selection_params.get('selection_strategy', 'rel'),
            selection_chunk_size=self.selection_params.get('selection_chunk_size', 0),
            div_lambda=self.selection_params.get('div_lambda', 0.1),
            div_candidate_ratio=self.selection_params.get('div_candidate_ratio', 3),
            div_feature_source=self.selection_params.get('div_feature_source', 'current_model'),
    ...
    def update_buffer(self, task_cnts, task_id, cur_x, cur_y, full_cur_x, full_cur_y, cur_id2logit=None,
        ...
        if self.selection_params.get('selection_strategy', 'rel') == 'rel_gss_anchor_replace':
