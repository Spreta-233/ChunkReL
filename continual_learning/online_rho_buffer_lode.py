...
class OnlineRhoBuffer(object):
    ...
    def update_buffer(self, d_ids, sps, labs, logits, ce_loss):
        ...
        if self.add_mode == 'rho_loss':
            ...
            for i in range(len(sps)):
                if logits is None:
                    self.update_buffer_random(sp=sps[i], lab=labs[i], logit=None)
                else:
                    self.update_buffer_random(sp=sps[i], lab=labs[i], logit=logits.numpy()[i, :])
        else:
    ...
    def get_sub_data_by_class(self, data_size, class_set):
        pos_pool = []
        for i, di in enumerate(self.data):
            lab = int(di[2])
            if lab in class_set:
                pos_pool.append(i)
        inds = random.sample(pos_pool, min(data_size, len(pos_pool)))
        selected_sps = []
        selected_labs = []
        selected_logits = []
        for idx in inds:
